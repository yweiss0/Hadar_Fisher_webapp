...
def load_and_plot_graph(graph_index):
    ...
    with left_col:
        ...
        outcome = st.selectbox(
            f"Outcome ({graph_index})",
            ["Negative Affect", "Angry", "Nervous", "Sad"],
            key=f"outcome_{graph_index}",
        ).lower()
        outcome = "na" if outcome == "negative affect" else outcome
        nomothetic_idiographic = st.selectbox(
            "Idiographic/Nomothetic",
            ["Idiographic", "Nomothetic"],
            key=f"nom_idio_{graph_index}",
        )
        ml_model = st.selectbox(
            f"ML Model ({graph_index})",
            ["Elastic Net (en)", "Random Forest (rf)"],
            key=f"ml_model_{graph_index}",
        )
        ...
        if file_path and os.path.exists(file_path):
            ...
            selected_participant = st.selectbox(
                f"Participant ({graph_index})",
                participants,
                key=f"participant_{graph_index}",
            )
            ...
            st.warning(
                "⚠️ The selected properties have already been used."
    ...
    with right_col:
        ...
        correlation = (
            np.corrcoef(na_levels, na_predicted)[0, 1] if len(na_levels) > 1 else np.nan
        )
...
new_app_chatbot.show_chatbot_ui(page_name="True vs Predicted")
